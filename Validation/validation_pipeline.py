...
from config import HasaimConfiguration, iter_list
...
class ValidationPipeline:
    ...
    def run(self) -> Dict[str, Dict[str, Any]]:
        ...
        results["static_analysis"] = self.static_analysis.run(
            ...
            static_analyzer=self.config.list_item("static_analyzer")
        )
        ...
        if exe_path is None:
            ...
            results["dynamic_analysis"] = self.dynamic_analysis.run(
                ...
                flags=self.config["flags"].split() if self.config["flags"] else []
            )
            status = "OK" if results["dynamic_analysis"].get("success") else "FAILED"
